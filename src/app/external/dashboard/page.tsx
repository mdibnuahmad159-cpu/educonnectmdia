...
import type { ExternalSaver, SavingsTransaction } from "@/types";
...
export default function ExternalDashboardPage() {
    ...
    useEffect(() => {
        setNip(sessionStorage.getItem('externalNip'));
    }, []);

    const saverRef = useMemoFirebase(() => nip && firestore ? doc(firestore, "externalSavers", nip) : null, [firestore, nip]);
    const { data: saver, loading: isSaverLoading } = useDoc<ExternalSaver>(saverRef);

    const savingsQuery = useMemoFirebase(() => {
        if (!firestore || !nip) return null;
        return query(
    ...
    if (!nip || loadingSavings || isSaverLoading) {
        return (
            ...
            {savingsError && (
                <Card className="border-destructive/20 bg-destructive/5">
                    <CardContent className="p-3 text-[10px] text-destructive flex items-center gap-2">
                        ...
                        Gagal memuat data tabungan. Silakan hubungi Madrasah.
                    </CardContent>
                </Card>
            )}
            ...
            <Card className="border-none shadow-md bg-primary text-primary-foreground overflow-hidden relative">
                <CardContent className="p-6 flex items-center justify-between relative z-10">
                    <div className="space-y-1">
                        <p className="text-[10px] uppercase font-bold opacity-80 tracking-widest">Saldo Terkini</p>
                        ...
                        <div className="flex items-center gap-2 mt-3 opacity-90">
                            ...
                            <div className="flex flex-col">
                                <span className="text-xs font-bold leading-tight">{saver?.name}</span>
                                <span className="text-[9px] font-mono opacity-70">NIP: {saver?.nip}</span>
                            </div>
                        </div>
                    </div>
                ...
                <Card className="border-none shadow-sm bg-green-50">
                    <CardContent className="p-4 flex flex-col gap-1">
                        <div className="flex items-center gap-2 text-green-600 mb-1">
                            ...
                            <span className="text-[9px] font-bold uppercase tracking-tight">Setoran</span>
                        </div>
                ...
                <Card className="border-none shadow-sm bg-red-50">
                    <CardContent className="p-4 flex flex-col gap-1">
                        <div className="flex items-center gap-2 text-red-600 mb-1">
                            ...
                            <span className="text-[9px] font-bold uppercase tracking-tight">Penarikan</span>
                        </div>
                ...
                <CardContent className="p-0">
                    {sortedTransactions.length > 0 ? (
                        ...
                        <div className="py-16 text-center text-muted-foreground italic">
                            ...
                            <p className="text-[10px]">Belum ada riwayat transaksi yang tercatat.</p>
                        </div>
                    )}
                </CardContent>
                <CardFooter className="bg-muted/5 border-t py-3">
                    <p className="text-[9px] text-muted-foreground italic leading-relaxed text-center w-full">
                        * Data tabungan dikelola oleh Bendahara Madrasah.
                    </p>
                </CardFooter>
            </Card>
        </div>
