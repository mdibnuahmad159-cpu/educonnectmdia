...
export default function ExternalAnnouncementsPage() {
    ...
    const filteredAnnouncements = useMemo(() => {
        ...
        // External savers see only 'Semua' announcements
        return allAnnouncements.filter(a => a.target === 'Semua');
    }, [allAnnouncements]);
                        ...
                        <Card key={item.id} className="border-none shadow-sm overflow-hidden rounded-[24px]">
                            ...
                            <CardFooter className="bg-muted/10 px-5 py-3 border-t">
                                <div className="flex items-center gap-2 text-[9px] font-bold text-muted-foreground uppercase tracking-widest">
                                    <Megaphone className="h-3 w-3" /> Informasi Umum
                                </div>
                            </CardFooter>
                        </Card>
                    ))
