...
const PAGE_TITLES: Record<string, { title: string; sub: string }> = {
  "/external/dashboard": { title: "Tabungan Saya", sub: "Pantau saldo dan riwayat mutasi." },
  "/external/announcements": { title: "Pengumuman", sub: "Informasi resmi dari Madrasah." },
};
...
export default function ExternalLayout({ children }: { children: ReactNode }) {
  ...
  // Cek pengumuman baru untuk penabung luar (hanya target 'Semua')
  ...
  useEffect(() => {
    if (allAnnouncements && allAnnouncements.length > 0) {
      const publicAnnouncements = allAnnouncements.filter(a => a.target === 'Semua');
      
      if (publicAnnouncements.length > 0) {
        const latestId = publicAnnouncements[0].id;
        const lastReadId = localStorage.getItem('last_read_announcement_external');
        
        if (latestId !== lastReadId && pathname !== "/external/announcements") {
          setHasNewAnnouncement(true);
        } else if (pathname === "/external/announcements") {
          setHasNewAnnouncement(false);
          localStorage.setItem('last_read_announcement_external', latestId);
        }
      }
    }
  ...
  const handleLogout = async () => {
    if (!auth) return;
    try {
      sessionStorage.removeItem('externalNip');
      await signOut(auth);
  ...
  const currentPage = useMemo(() => {
    return PAGE_TITLES[cleanPath] || { title: "Penabung Umum", sub: "Layanan Tabungan Madrasah." };
  }, [cleanPath]);

  const isDashboard = cleanPath === "/external/dashboard";
          ...
          <div className="px-4 py-2 flex items-center justify-between min-h-[48px]">
              <div className="w-12 flex justify-start items-center gap-1">
                  {!isDashboard ? (
                    ...
                    <Link href="/external/announcements" className="relative p-1.5 text-accent hover:bg-white/10 rounded-full transition-all active:scale-90">
