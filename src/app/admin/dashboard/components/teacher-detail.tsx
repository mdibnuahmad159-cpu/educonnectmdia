...
export function TeacherDetail({ isOpen, setIsOpen, teacher, onEdit, onDelete }: TeacherDetailProps) {
  ...
  const handleDownloadIDCardPDF = async () => {
    ...
    doc.text("KARTU IDENTITAS GURU", 37, 21, { align: 'center' });

    // Photo
    if (teacher.avatarUrl) {
        try {
            doc.setFillColor(255, 255, 255);
            doc.circle(37, 35, 10, 'F'); // Photo container circle
            doc.addImage(teacher.avatarUrl, 'JPEG', 28, 26, 18, 18);
        } catch (e) {}
    }
    ...
    doc.setFontSize(9);
    doc.setFont("helvetica", "bold");
    doc.text(cleanedName.toUpperCase(), 37, 50, { align: 'center', maxWidth: 50 });

    doc.setFontSize(8);
    doc.setFont("helvetica", "normal");
    doc.text(teacher.nig, 37, 55, { align: 'center' });

    // Jabatan
    doc.setFontSize(6.5);
    doc.text(teacher.jabatan?.toUpperCase() || 'PENDIDIK', 37, 61, { align: 'center' });
    ...
    doc.save(`ID_Card_Guru_${cleanedName.replace(/\s+/g, '_')}.pdf`);
  };
